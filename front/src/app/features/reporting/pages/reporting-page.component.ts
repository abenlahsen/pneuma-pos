    ...
    return [
      {
        ...
        title: 'Ventes',
        cards: [
          { icon: 'banknote', label: 'CA Ventes', value: c.sales.total, prev: p.sales.total, kind: 'money', tone: 'up-good', variant: 'red',
            sub: `${fmtInt(c.sales.count)} vente(s) · panier moyen ${fmtMoney(c.sales.avg_basket)}` },
          { icon: 'receipt', label: 'Avec facture', value: c.sales.with_invoice, prev: p.sales.with_invoice, kind: 'money', tone: 'up-good' },
          { icon: 'edit', label: 'Sans facture', value: c.sales.without_invoice, prev: p.sales.without_invoice, kind: 'money', tone: 'up-good' },
          { icon: 'package', label: 'Pneus vendus', value: c.sales.tyres_qty, prev: p.sales.tyres_qty, kind: 'int', tone: 'up-good',
            sub: `${fmtInt(c.sales.parts_qty)} pièce(s) vendue(s)` },
          { icon: 'bar-chart', label: 'Prix moyen / pneu', value: c.sales.avg_price_per_tyre, prev: p.sales.avg_price_per_tyre, kind: 'money', tone: 'up-good' },
          { icon: 'clock', label: 'Impayés générés', value: c.sales.unpaid_generated, prev: p.sales.unpaid_generated, kind: 'money', tone: 'up-bad', variant: 'warning',
      ...
      },
      {
        ...
        title: 'Achats',
        cards: [
          { icon: 'package', label: 'Achats nets', value: c.purchases.total, prev: p.purchases.total, kind: 'money', tone: 'neutral', variant: 'blue',
            sub: `${fmtInt(c.purchases.count)} achat(s) · après remises et retours` },
          { icon: 'receipt', label: 'Avec facture', value: c.purchases.with_invoice, prev: p.purchases.with_invoice, kind: 'money', tone: 'neutral' },
          { icon: 'edit', label: 'Sans facture', value: c.purchases.without_invoice, prev: p.purchases.without_invoice, kind: 'money', tone: 'neutral' },
          { icon: 'package', label: 'Pneus achetés', value: c.purchases.tyres_qty, prev: p.purchases.tyres_qty, kind: 'int', tone: 'neutral',
            sub: `${fmtInt(c.purchases.parts_qty)} pièce(s) achetée(s)` },
          { icon: 'undo', label: 'Retours fournisseurs', value: c.purchases.returns_amount, prev: p.purchases.returns_amount, kind: 'money', tone: 'neutral',
            sub: `${fmtInt(c.purchases.returns_count)} retour(s) · ${fmtMoney(c.purchases.refunds_received)} remboursés` },
          { icon: 'clock', label: 'Impayés générés', value: c.purchases.unpaid_generated, prev: p.purchases.unpaid_generated, kind: 'money', tone: 'up-bad', variant: 'warning',
      ...
      },
      {
        ...
        title: 'Marge',
        cards: [
          { icon: 'trending-up', label: 'Marge brute', value: c.margin.gross, prev: p.margin.gross, kind: 'money', tone: 'up-good', variant: 'red',
            sub: `Taux de marge ${this.formatPercent(c.margin.rate)} %` },
          { icon: 'tag', label: 'Marge ventes', value: c.margin.sales, prev: p.margin.sales, kind: 'money', tone: 'up-good' },
          { icon: 'wrench', label: 'Marge Service Auto', value: c.margin.service, prev: p.margin.service, kind: 'money', tone: 'up-good' },
          { icon: 'banknote', label: 'Autres revenus', value: c.margin.other_revenue, prev: p.margin.other_revenue, kind: 'money', tone: 'up-good' },
          { icon: 'banknote', label: 'Charges', value: c.margin.expenses, prev: p.margin.expenses, kind: 'money', tone: 'up-bad', variant: 'warning' },
          { icon: 'target', label: 'Marge nette', value: c.margin.net, prev: p.margin.net, kind: 'money', tone: 'up-good', variant: 'red',
      ...
      },
      {
        ...
        cards: [
          { icon: 'credit-card', label: 'Encaissements ventes', value: c.collections.total, prev: p.collections.total, kind: 'money', tone: 'up-good', variant: 'red',
            sub: c.collections.unallocated > 0 ? `dont ${fmtMoney(c.collections.unallocated)} non affectés` : undefined },
          { icon: 'receipt', label: 'Encaissé avec facture', value: c.collections.with_invoice, prev: p.collections.with_invoice, kind: 'money', tone: 'up-good' },
          ...
          { icon: 'wrench', label: 'Encaissements Service Auto', value: c.collections.service_orders, prev: p.collections.service_orders, kind: 'money', tone: 'up-good' },
          { icon: 'building', label: 'Paiements fournisseurs', value: c.supplier_payments.total, prev: p.supplier_payments.total, kind: 'money', tone: 'neutral', variant: 'blue',
            sub: c.supplier_payments.unallocated > 0 ? `dont ${fmtMoney(c.supplier_payments.unallocated)} non affectés` : undefined },
          { icon: 'receipt', label: 'Payé avec facture', value: c.supplier_payments.with_invoice, prev: p.supplier_payments.with_invoice, kind: 'money', tone: 'neutral' },
      ...
      },
      {
        ...
        cards: [
          { icon: 'user', label: 'Masse salariale', value: c.payroll.total, prev: p.payroll.total, kind: 'money', tone: 'up-bad', variant: 'warning',
      ...
      },
      {
        ...
        title: 'Service Auto',
        cards: [
          { icon: 'wrench', label: 'CA Service Auto', value: c.service_orders.revenue, prev: p.service_orders.revenue, kind: 'money', tone: 'up-good', variant: 'red',
            sub: `${fmtInt(c.service_orders.count)} ordre(s) de service` },
          { icon: 'trending-up', label: 'Marge Service Auto', value: c.service_orders.gross_margin, prev: p.service_orders.gross_margin, kind: 'money', tone: 'up-good' },
          { icon: 'credit-card', label: 'Encaissements', value: c.service_orders.collected, prev: p.service_orders.collected, kind: 'money', tone: 'up-good' },
          { icon: 'package', label: 'Pneus posés', value: c.service_orders.tyres_qty, prev: p.service_orders.tyres_qty, kind: 'int', tone: 'up-good' },
        ],
      },
      {
        key: 'cash',
        title: 'Trésorerie',
        cards: [
          { icon: 'trending-up', label: 'Entrées réalisées', value: c.cash_flow.income_settled, prev: p.cash_flow.income_settled, kind: 'money', tone: 'up-good' },
          { icon: 'trending-down', label: 'Sorties réalisées', value: c.cash_flow.expense_settled, prev: p.cash_flow.expense_settled, kind: 'money', tone: 'up-bad' },
          { icon: 'banknote', label: 'Flux net', value: c.cash_flow.net, prev: p.cash_flow.net, kind: 'money', tone: 'up-good', variant: 'red',
            sub: 'Hors transferts entre comptes' },
          { icon: 'clock', label: 'Échéances à recevoir', value: c.cash_flow.pending_income, prev: p.cash_flow.pending_income, kind: 'money', tone: 'neutral',
            sub: 'Chèques / effets non échus' },
          { icon: 'clock', label: 'Échéances à payer', value: c.cash_flow.pending_expense, prev: p.cash_flow.pending_expense, kind: 'money', tone: 'neutral',
            sub: 'Chèques / effets non échus' },
