...
# fails with "SyntaxError: Unexpected token" under Node 18) — and if nvm's
# active version is set to "system", a non-interactive shell like this one
# silently picks up that old apt-installed Node instead of an nvm version.
