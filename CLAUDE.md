...
**Shared** (`front/src/app/shared/`): Cross-feature reusable components — `auto-refresh-control` (per-page configurable auto-refresh toggle), `rail` (64px navigation rail + overlaid section panel, replaces the former `navbar`; `rail/nav-items.ts` holds the shared `NAV_ITEMS` destination tree, consumed by both `RailComponent` and `command-palette`), `icon` (`<app-icon name="…">`, inline Lucide SVGs), `command-palette` (`Ctrl/⌘ K` search overlay — destinations + `GET /api/sales?search=` + `GET /api/products?search=`, mounted once in `app.html`), `document-print` (A4 PDF preview modal for sales, purchases, and service orders).
