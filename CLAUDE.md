...
- `admin.php` — CRUD for `users` and `roles`/`permissions` + `GET /api/dashboard-kpi` (Administrator only) + `GET /api/primes-commerciaux` (permission `view primes`) + `GET /api/kpi-history` (daily KPI snapshots, Administrator only) + `GET /api/reporting/monthly` (monthly management report, permission `view reporting`); `require`s `settings.php`
...
- Modules: `sales`, `purchases`, `cash-flow`, `clients`, `suppliers`, `users`, `carriers`, `partners`, `stock`, `roles`, `brands`, `products`, `accounts`, `settings`, `service-orders`, `activity-log`, `vehicles`, `primes`, `kpi-history`, `reporting`

**Shared** (`front/src/app/shared/`): Cross-feature reusable components — `auto-refresh-control` (per-page configurable auto-refresh toggle), `rail` (72px dark navigation rail; `rail/nav-items.ts` holds the shared `NAV_ITEMS` destination tree; a destination with `children` opens an overlaid flyout panel next to the rail — one open at a time — instead of a horizontal dropdown), `topbar` (60px light top bar: logo, a currently-inert search box reserved for a future command palette, live clock, user avatar with a Déconnexion dropdown), `icon` (`<app-icon name="…">`, inline Lucide SVGs; `shared/icon/sort-icon.component.ts` renders list-column sort indicators), `document-print` (A4 PDF preview modal for sales, purchases, and service orders). The former `shared/navbar/` (horizontal/vertical layout toggle via `menuLayout`) was replaced by `rail`+`topbar` (refonte 2b, étape 3) — `menuLayout` no longer drives any layout in `app.html`/`app.scss`. `CompanySetting.menu_layout`/`navbar_variant` and their controls on the company settings page still exist but are now inert leftovers, not yet cleaned up.
...
**Detail/consultation modals**: All three modules' detail components (`sale-detail`, `purchase-detail`, `service-order-detail`) expose `@Input() canEdit = false` and `@Output() edit`. The parent page passes `[canEdit]="authService.hasPermission('edit X')"` and `(edit)="editFromDetail()"`. The `editFromDetail()` method closes the detail modal then immediately opens the edit form. `sale-detail` and `purchase-detail` also expose `@Input() hasPrev / hasNext / position` and `@Output() prev / next` for **Précédent / Suivant** navigation inside the modal (small ‹ › arrow buttons with the record number (`N° {id}`) and the position counter in the sticky modal header, next to the title — the footer only holds the action buttons): the parent page owns a `DetailNavigator` (`core/utils/detail-navigator.ts`, pure signals, no DI) that steps through the loaded page in table order and, at a page edge, calls `goToPage()` then opens the first/last row of the new page from `loadData()`'s `next` handler via `detailNav.onListLoaded()` (`detailNav.reset()` on error/close). Because the modal instance is kept by `*ngIf` while the input is swapped, both detail components implement `ngOnChanges` to reset their per-record state and reload shipment requests / supplier returns. ← / → are handled by `@HostListener('document:keydown')`, ignored when typing in a field (`isTypingTarget()`) or when a nested panel (print preview, product view, shipment form) is open.
...
**Reporting mensuel** (`GET /api/reporting/monthly?year=&month=`, permission `view reporting` — Administrator only, explicitly excluded from Manager): one-month management report computed by `Domain/Reporting/MonthlyReportService::build()`. Returns `{ period, previous_period, current, previous }` where `current`/`previous` share the same shape (`sales`, `purchases`, `service_orders`, `margin`, `collections`, `supplier_payments`, `expenses`, `payroll`, `cash_flow`, `commercials`, `top_brands`, `clients`); the frontend computes the M-1 deltas. Rules: all figures use the entity `date` column and exclude `ANNULE` rows (same as the dashboard); **sales/purchases with/without invoice** come from `with_invoice`; **collections/supplier payments with/without invoice** are split through `sale_payment_allocations` / `purchase_payment_allocations` (the payment tables have no invoice flag — the remainder is exposed as `unallocated`); **purchases are net** (`net_amount - returned_amount * (1 - discount/100)`); **payroll** reuses `HrChargeService::summary()` and is a breakdown of `expenses` (the `Charges RH` category already counts as an expense), not an addition; `cash_flow` excludes the `Transfert` system category. Service Auto is reported as its own block (no invoice flag on service orders). Stock value and account balances are deliberately absent (point-in-time). Aggregates use `DB::table()` rather than Eloquent because `Sale` has accessors (`total`, `subtotal`) that shadow SQL aliases. Frontend page at `/reporting` (arrow month stepper, KPI cards with M-1 delta badges, tables for expenses by category, payroll by subcategory, payment methods, commercials, top brands).

**Activity Log** (`GET /api/activity-logs`, Administrator only): append-only audit trail for sales, purchases, and service orders. Actions logged: CREATE (entity created), UPDATE (diff of changed fields), DELETE (snapshot of deleted entity), PAYMENT_ADD, PAYMENT_DELETE. Written by `ActivityLogService` injected into all domain services — same pattern as `StockMovementService`. `deletePayment()` methods on all three payment services accept `?int $userId` and `?string $userName` optional params (passed from their controllers). `user_name` is denormalized so attribution survives user deletion. `properties` JSON: diff object `{field: {from, to}}` for UPDATE, snapshot array for DELETE, `{amount, method, reference}` for payment events. Frontend at `/activity-log`, visible to Administrator only, in the rail's Administration flyout.
